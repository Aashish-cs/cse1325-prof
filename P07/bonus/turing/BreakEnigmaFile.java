...
public class BreakEnigmaFile {
    ...
    // Rewrite this section to split the work between numThreads threads!
        ...
        nextIndex = encrypteds.size() - 1;
        ArrayList<Thread> threads = new ArrayList<>();
        
        // Create worker threads to solve each of the encrypted pairs
        for(int i=0; i<numThreads; ++i) {
            final int threadID = i;
            final BreakEnigmaFile breaker = new BreakEnigmaFile();
            Thread t = new Thread(() -> breaker.breakManager(threadID));
            t.start();
            threads.add(t);
        }
        for(Thread t : threads) {
            try {
                ...
                System.err.println("WARNING: InterruptedException thrown!");
            }
        }
    ...
    // This little synchronized method doles out indices to the encrypteds array list
    //   from last to first (because the last take much longer than the first).
    // The return value will naturally go negative once all strings have been doled out.
    ...
    public void breakManager(int threadID) {
